...
with col_details:
    ...
    if display_ticker:
        ...
        for i, tf in enumerate(timeframes):
            if cols[i].button(tf, use_container_width=True, key=f"tf_{tf}"):
        ...
        with st.spinner("Loading chart data..."):
            try:
                ...
                hist = stock.history(period="30d", interval=st.session_state.chart_timeframe)
                ...
                if not hist.empty:
                    ...
                    fig.update_layout(
                        title=f"{display_ticker} - {st.session_state.chart_timeframe.upper()}",
                        height=550,
...
st.divider()
st.info("💡 Click any row in the table to auto-load the chart. Use timeframe buttons above.")
