...
st.caption("Modular Scanner | Real-time Ready")
...
with st.sidebar:
    ...
    scanner_type = st.radio("Choose Data Source", ["yFinance (Free & Working)", "Massive.io (Paid)"], index=0)
    ...
    risk_percent = st.slider("Risk %", 1.0, 10.0, 5.0)
...
with col_table:
    ...
    if st.button("🔄 Run Live Scan", type="primary", use_container_width=True):
        with st.spinner(f"Scanning with {scanner_type}..."):
            if "yFinance" in scanner_type:
...
with col_details:
    ...
    if st.button("Load Chart + Zones") and ticker:
        st.info(f"Loading {ticker}...")
        fig = go.Figure()
        fig.update_layout(title=f"{ticker} Chart", height=480, template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)

st.divider()
st.info("💡 yFinance is currently more reliable. Upgrade Massive for real-time power.")
